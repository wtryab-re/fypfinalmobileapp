...
router.post("/", upload.single("image"), async (req, res) => {
  try {
    const { patientId, patientHistory, manualChecks, uploadedBy } = req.body;
...
router.get("/detail/:caseId", async (req, res) => {
  try {
    ...
    const caseData = await Case.findById(caseId).populate("aiResult").lean();
