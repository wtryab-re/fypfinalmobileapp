...
import { Dimensions, Image, StyleSheet, Text, TextInput, TouchableOpacity, View, ActivityIndicator, ScrollView } from 'react-native';
...
const HealthWorkerDashboard: React.FC = () => {
  ...
  const fetchRecentCases = async (): Promise<void> => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/worker/recent-cases`, {
        headers: { token }
      });
  ...
  <SafeAreaView style={styles.container}>
    ...
    >
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.hwName}>{workerData?.name || 'Health Worker'}</Text>
          <Text style={styles.hwId}>HW-{workerData?._id?.slice(-8).toUpperCase() || 'UNKNOWN'}</Text>
        </View>
        ...
        >
          <Ionicons name="person-circle-outline" size={width * 0.13} color="#1a78d2" />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.recentRecordsHeader}>
        ...
        <TouchableOpacity onPress={() => router.push('/startscreens/AllCases' as any)}>
      ...
      {recentCases.length > 0 ? (
          ...
          {recentCases.map((caseItem, index) => {
            ...
            return (
              ...
              >
                ...
                <Text style={[styles.recordStatus, { color: statusInfo.color }]}>
                  {statusInfo.text}
                </Text>
              </TouchableOpacity>
            );
        ...
        <View style={styles.noCasesContainer}>
          ...
          <Text style={styles.noCasesSubtext}>Submit your first case to get started</Text>
        </View>
      )}
    ...
    <View style={styles.bottomNav}>
      ...
      <TouchableOpacity onPress={() => router.push('/startscreens/profile' as any)}>
